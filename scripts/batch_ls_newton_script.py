...
from optsmooth.sequential._ls_newton import _iterated_batch_newton_smoother
...
_, true_states, observations = get_data(x0, dt, r, T, s1, s2, random_state=13)
...
plt.subplot(1, 2, 2)
plt.plot(newton_smoothed[:, 0], newton_smoothed[:, 1], "-*", label="Iterated Batch Newton Smoother")
plt.plot(true_states[:, 0], true_states[:, 1], "*", label="True")
plt.title("Newton")
plt.grid()
plt.legend()
