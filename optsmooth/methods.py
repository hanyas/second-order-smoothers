...
def filtering(observations: jnp.ndarray,
    ...
    return seq_filtering(observations, init_dist,
                         transition_model, observation_model, linearization_method,
...
def filter_smoother(observations: jnp.ndarray,
    ...
    filter_trajectory = filtering(observations,
                                  init_dist, transition_model, observation_model,
                                  linearization_method, nominal_trajectory)
    return smoothing(transition_model, filter_trajectory, linearization_method, nominal_trajectory)


def _default_criterion(_i, prev_nominal_traj, curr_nominal_traj):
    return jnp.mean((prev_nominal_traj.mean - curr_nominal_traj.mean) ** 2) > 1e-6
...
def iterated_smoothing(observations: jnp.ndarray,
    ...
    if init_nominal_trajectory is None:
        init_nominal_trajectory = filter_smoother(observations,
                                                  init_dist, transition_model, observation_model,
                                                  linearization_method, None)

    def fun_to_iter(curr_nominal_traj):
        return filter_smoother(observations,
                               init_dist, transition_model, observation_model,
                               linearization_method, curr_nominal_traj)

    nominal_traj = fixed_point(fun_to_iter, init_nominal_trajectory, criterion)
    if return_loglikelihood:
        _, ell = filtering(observations,
                           init_dist, transition_model, observation_model,
                           linearization_method, nominal_traj, return_loglikelihood=True)
